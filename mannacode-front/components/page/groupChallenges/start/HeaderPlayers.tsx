...
export default function headerPlayers() {
  ...
  useEffect(() => {
    client.service('status-players').on('created', statusPlayers => {
      setStatusJoinPlayer(statusPlayers.data.players);
    });
  }, [])
  ...
  return (
    <Grid item xs={6} sm={3} md={2} style={{background: paletteCode.get().head}}>
      ...
      <Typography className={classes.headPlayers} style={{ color: paletteCode.get().color }}>Jogadores</Typography>
      <Box>
