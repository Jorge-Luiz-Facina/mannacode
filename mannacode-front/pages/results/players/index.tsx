...
import { getPlayersByApplicatorStartId } from '@/services/player';
...
function Players(props) {
  ...
  const columns: GridColDef[] = [
    ...
    {
      field: 'punctuationTotal', headerAlign: 'center', align: 'center', headerName: 'Pontuação',
      width: width !== 'xs' ? field10 : 150, description: 'Pontuação do jogador',
      sortable: false,
        ...
        >
          ...
          <Grid item xs={5} style={{ textAlign: 'end' }}>
            <IconButton className={classesTheme.iconItem}
              ...
              aria-haspopup="true"
              onClick={() => { router.push(`${PathName.results}${PathName.players}${PathName.challenges}?groupChallengeId=${groupChallengeId}&playerStartId=${params.row.id}`) }}
            >
              <ArrowForwardIosIcon />
            </IconButton>
          </Grid>
  ...
  useEffect(() => {
    ...
    if (signed && page) {
      setLoading(true);
      getPlayersByApplicatorStartId({ ...page, groupChallengeId }, token)
        .then((response) => {
        ...
        >
          <Grid item xs={12} sm={10} style={{ background: palette.get().secondaryText }}>
            ...
            <div style={{ height: window.innerHeight / 100 * 80, width: '100%' }}>
              <DataGrid onPageChange={handlePageChange} rowCount={total} textNoRowsOverlay={'Não há jogadores'} onSortModelChange={onSortModelChange}
