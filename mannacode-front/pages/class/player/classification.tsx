...
import {  getPunctuationPlayersFinalized } from '@/services/challengePlayersFinalized';
...
function Classification(props) {
  ...
  const columns: GridColDef[] = [
    ...
    {
      field: 'punctuationTotal', headerAlign: 'center', align: 'center', type: 'date', headerName: 'Pontuação', width: width !== 'xs' ? field10 : 150, description: 'Pontuação do jogador sobre a atividade',
    },
  ];
  ...
  useEffect(() => {
    ...
    if (page) {
      setLoading(true);
      getPunctuationPlayersFinalized(page, getLocalStorage(LocalStorageItem.tokenPlayer))
        .then((response) => {
          if (response.ok) {
